...
import { Activity, Radio, LineChart as ChartLine, Settings as Gear, Zap as Lightning } from 'lucide-react';
...
export default function Layout({ children }) {
  ...
  const section = NAV.find(n => n.to === loc.pathname)?.label || 'Live Triage';
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex">
      ...
      <aside className="w-60 border-r border-[#1f1f1f] bg-[#0A0A0A] flex flex-col">
        ...
        <div className="mt-auto p-3 border-t border-[#1f1f1f] text-[10px] text-neutral-500">
          ...
          <div className="mt-1 text-neutral-600">v0.1.0 · MVP</div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-[#1f1f1f] px-6 flex items-center justify-between bg-[#0A0A0A]/95 backdrop-blur sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2 text-[10px] tracking-widest uppercase text-neutral-500">
            ...
            <span className="px-2 py-1 border border-[#262626]">multi-region</span>
          </div>
        </header>
